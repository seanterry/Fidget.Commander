...
{
    public class CommandAdapter_TCommand_Tests
    {
        public class TestCommand : ICommand {}

        Mock<ICommandHandler<TestCommand>> mockHandler = new Mock<ICommandHandler<TestCommand>>();

        ICommandHandler<TestCommand> handler => mockHandler?.Object;
        List<ICommandDecorator<TestCommand>> decorators = new List<ICommandDecorator<TestCommand>>();
        ICommandAdapter instance => new CommandAdapter<TestCommand>( handler, decorators );

        public class Constructor : CommandAdapter_TCommand_Tests
        {
            [Fact]
        ...
        public class Execute : CommandAdapter_TCommand_Tests
        {
            ...
            class FakeDecorator : ICommandDecorator<TestCommand>
            {
                public Action<TestCommand> callback;

                public async Task Execute( TestCommand command, CancellationToken cancellationToken, CommandDelegate<TestCommand> continuation )
                {
            ...
            public async Task calls_handler_and_decorators( int expectedDecorators )
            {
                ...
                {
                    decorators.Add( new FakeDecorator { callback = actualCommand =>
                    {
